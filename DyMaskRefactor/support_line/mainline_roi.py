...
from DyMaskRefactor.support_line.mainline_hardcore import RefactorSupportC5H0Editor
from DyMaskRefactor.support_line.schedules import cosine_schedule, schedule_progress, validate_unit_interval


class RefactorSupportC5H2Editor(RefactorSupportC5H0Editor):
    def __init__(
        self,
        pipe,
        config,
        ...
        self.core_read_start_weight = validate_unit_interval(core_read_start_weight, "core_read_start_weight")
        self.core_read_end_weight = validate_unit_interval(core_read_end_weight, "core_read_end_weight")
        self.boundary_read_start_weight = validate_unit_interval(boundary_read_start_weight, "boundary_read_start_weight")
        self.boundary_read_end_weight = validate_unit_interval(boundary_read_end_weight, "boundary_read_end_weight")
        self.boundary_anchor_start_weight = validate_unit_interval(boundary_anchor_start_weight, "boundary_anchor_start_weight")
        self.boundary_anchor_end_weight = validate_unit_interval(boundary_anchor_end_weight, "boundary_anchor_end_weight")
        self.boundary_confidence_weight = validate_unit_interval(boundary_confidence_weight, "boundary_confidence_weight")
        self.roi_core_quantile = validate_unit_interval(roi_core_quantile, "roi_core_quantile")
        self.roi_core_peak_ratio = validate_unit_interval(roi_core_peak_ratio, "roi_core_peak_ratio")
        self.roi_core_threshold_min = validate_unit_interval(roi_core_threshold_min, "roi_core_threshold_min")
        self.roi_core_threshold_max = validate_unit_interval(roi_core_threshold_max, "roi_core_threshold_max")
        self.roi_core_min_active_ratio = validate_unit_interval(roi_core_min_active_ratio, "roi_core_min_active_ratio")
        self.roi_core_active_floor = validate_unit_interval(roi_core_active_floor, "roi_core_active_floor")
    ...
    def _soft_roi_weight(self, step_idx: int, total_steps: int) -> float:
        return self._boundary_read_weight(step_idx, total_steps)

    def _anchor_hardness(self, step_idx: int, total_steps: int) -> float:
        return 1.0 - self._boundary_anchor_weight(step_idx, total_steps)
    ...
    def _extra_step_aux_tensors(
        ...
        boundary_mask = self._resolve_roi_boundary_mask(roi_mask)
        if core_mask is not None:
            tensors["roi_core_mask"] = core_mask
            tensors["hard_roi_mask"] = core_mask
        if self._current_soft_roi_mask is not None:
            tensors["soft_roi_mask"] = self._current_soft_roi_mask
        if boundary_mask is not None:
            tensors["roi_boundary_mask"] = boundary_mask
        if self._current_legacy_hard_roi_mask is not None:
            tensors["legacy_hard_roi_mask"] = self._current_legacy_hard_roi_mask
        if self._current_roi_core_threshold is not None:
            tensors["roi_core_threshold"] = self._current_roi_core_threshold.expand_as(roi_mask)
        tensors["core_read_weight"] = torch.full_like(roi_mask, self._core_read_weight(step_idx, total_steps))
        tensors["boundary_read_weight"] = torch.full_like(roi_mask, self._boundary_read_weight(step_idx, total_steps))
        tensors["boundary_anchor_weight"] = torch.full_like(roi_mask, self._boundary_anchor_weight(step_idx, total_steps))
        tensors["boundary_confidence_weight"] = torch.full_like(roi_mask, self.boundary_confidence_weight)
        return tensors
