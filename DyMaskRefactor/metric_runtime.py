...
from .third_party.dino import build_dino_model
...
class DinoVitExtractor:
    ...
    def __init__(self, model_name: str, device: str, weights_path: str | None = None) -> None:
        self.model = build_dino_model(model_name=model_name, weights_path=weights_path).to(device)
        self.model.eval()
...
class DinoStructureDistanceCalculator(torch.nn.Module):
    def __init__(self, metric_config: MetricConfig, device: str) -> None:
        super().__init__()
        self.device = device
        self.extractor = DinoVitExtractor(
            model_name=metric_config.dino_model_name,
            device=device,
            weights_path=metric_config.dino_weights_path,
        )
        imagenet_norm = transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
        self.global_transform = transforms.Compose(
            [
                Resize(metric_config.dino_global_patch_size, max_size=480),
                imagenet_norm,
            ]
        )
...
class MetricRunner:
    ...
    def _lazy_clip(self):
        ...
        if self.metric_config.clip_local_files_only:
            ...
            local_clip_path = Path(self.runtime.clip_model_id).expanduser()
            if local_clip_path.exists():
                processor_source = str(local_clip_path)
                model_source = str(local_clip_path)
            else:
    ...
    def _lazy_structure_distance(self):
        ...
        if not self.metric_config.dino_weights_path:
            raise FileNotFoundError(
                "Structure distance requires --dino-weights-path or metrics.dino_weights_path in config."
            )
