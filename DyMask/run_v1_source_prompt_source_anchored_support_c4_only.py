...
from DyMask.v1_source_prompt_source_anchored_support_c4_only import V1SourcePromptSourceAnchoredSupportC4OnlyEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c4_only"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c4_only"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c4_only"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c4_only.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c4_only_late_roi_floor_v1",
            ...
            "mechanism": "Baseline adaptive temporal support plus a late soft-roi floor only",
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_mask * dynamic_mask), S_0 = roi_mask * dynamic_mask",
            "adaptive_mask": "M_t = max(lerp(S_t, roi_soft, w_t), F_t), where F_t = alpha_t * roi_soft and alpha_t is a late cosine gate",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c4_only",
        ...
        conclusion="samples and C4-only source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC4OnlyEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C4-only source-anchored temporal-support prompt run finished",
        next_step="inspect whether the late roi floor alone reduces under-editing",
    )
