...
from DyMask.v1_source_prompt_source_anchored_support_c6 import V1SourcePromptSourceAnchoredSupportC6Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c6"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c6"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c6"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c6.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": (
                ...
                "late roi-trust fallback and a consistency floor. Support memory stays baseline; "
                "source anchoring relaxes where discrepancy confirms edits, and later can also relax "
                "where roi/support remain stable even if discrepancy is weak."
            ),
            ...
            "background_anchor": (
                ...
                "R_t = alpha_t * roi_soft * max(sqrt(discrepancy * dynamic_mask), beta_t * sqrt(max(support_state, mask))) "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c6",
        ...
        conclusion="samples and C6 source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC6Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C6 source-anchored temporal-support prompt run finished",
        next_step="inspect whether late roi-trust fallback recovers weak-discrepancy under-edits without losing locality",
    )
