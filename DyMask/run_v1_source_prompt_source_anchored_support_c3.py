...
from DyMask.v1_source_prompt_source_anchored_support_c3 import V1SourcePromptSourceAnchoredSupportC3Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c3"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c3"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c3"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c3.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": "Adaptive temporal support with delayed source anchoring and delayed soft-support injection so early support memory stays hard and soft roi enters only after editing has formed",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c3",
        ...
        conclusion="samples and C3 source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC3Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C3 source-anchored temporal-support prompt run finished",
        next_step="inspect summary tables and delayed soft-support behavior",
    )
