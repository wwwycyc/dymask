...
from DyMask.v1_source_prompt_source_anchored_hard_roi import V1SourcePromptSourceAnchoredHardRoiEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_hard_roi_runs/sp_anchor_hard_roi"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_anchor_hard_roi" if config.phase != "custom" else "sp_anchor_hard_roi"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_hard_roi_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI editing with outside-roi source latent anchoring",
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi",
        ...
        conclusion="samples and source-anchored hard-roi config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored hard-roi run finished",
