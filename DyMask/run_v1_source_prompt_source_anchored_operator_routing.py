...
from DyMask.v1_source_prompt_source_anchored_operator_routing import (
    V1SourcePromptSourceAnchoredOperatorRoutingEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--core-kernel-size", type=int, default=5)
    parser.add_argument("--disable-boundary-band", action="store_true")
    parser.add_argument("--rewrite-boundary-scale", type=float, default=0.5)
    parser.add_argument("--stabilize-scale", type=float, default=1.0)
    parser.add_argument("--preserve-scale", type=float, default=1.0)
    parser.add_argument("--disable-stabilize-operator", action="store_true")
    parser.add_argument("--frequency-kernel-size", type=int, default=5)
    parser.add_argument("--stabilize-low-source-weight", type=float, default=0.35)
    parser.add_argument("--stabilize-high-source-weight", type=float, default=0.75)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_operator_routing_runs/sp_operator_routing"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_operator_routing" if config.phase != "custom" else "sp_operator_routing"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_operator_routing.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_operator_routing_v1",
            ...
            "core_kernel_size": args.core_kernel_size,
            "use_boundary_band": not bool(args.disable_boundary_band),
            "rewrite_boundary_scale": args.rewrite_boundary_scale,
            "stabilize_scale": args.stabilize_scale,
            "preserve_scale": args.preserve_scale,
            "use_stabilize_operator": not bool(args.disable_stabilize_operator),
            "frequency_kernel_size": args.frequency_kernel_size,
            "stabilize_low_source_weight": args.stabilize_low_source_weight,
            "stabilize_high_source_weight": args.stabilize_high_source_weight,
    ...
    logger.log(
        stage="source_prompt_source_anchored_operator_routing",
        ...
        conclusion="samples and source-anchored operator-routing config saved",
    ...
    editor = V1SourcePromptSourceAnchoredOperatorRoutingEditor(
        pipe,
        config,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
        transform_attention_weight=args.transform_attention_weight,
        core_kernel_size=args.core_kernel_size,
        disable_boundary_band=args.disable_boundary_band,
        rewrite_boundary_scale=args.rewrite_boundary_scale,
        stabilize_scale=args.stabilize_scale,
        preserve_scale=args.preserve_scale,
        disable_stabilize_operator=args.disable_stabilize_operator,
        frequency_kernel_size=args.frequency_kernel_size,
        stabilize_low_source_weight=args.stabilize_low_source_weight,
        stabilize_high_source_weight=args.stabilize_high_source_weight,
    ...
    logger.log(
        ...
        conclusion="source-anchored operator-routing run finished",
        next_step="compare against hard-roi baseline and inspect boundary behavior",
    )
