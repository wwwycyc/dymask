...
class DiffEditEditor:
    ...
    def _run_diffedit(self, sample: MaterializedSample, inversion: InversionOutput) -> MethodResult:
        ...
        source_pil = self._load_sample_pil(sample.source_image_path)
        ...
        mask_latent = np.asarray(mask_output[0] if isinstance(mask_output, np.ndarray) and mask_output.ndim == 3 else mask_output)
        if mask_latent.ndim != 2:
            raise ValueError(f"Unexpected DiffEdit mask shape: {mask_latent.shape}")

        _timesteps = self._get_edit_timesteps()
        image_latents = self._prepare_image_latents(inversion, len(_timesteps))
        edited_output = self.pipe(
            prompt=sample.target_prompt,
        ...
        edited_image = np.asarray(edited_output.images[0].convert("RGB"))

        edited_path = method_dir / "edited.png"
        save_image(edited_path, edited_image)

        latent_mask_path, resized_mask_path = self._save_mask_outputs(method_dir, mask_latent, source_pil.size)
    ...
    def run_samples(self, samples: list[MaterializedSample]) -> dict[str, tuple[InversionOutput, list[MethodResult]]]:
        ...
        for sample in samples:
            ...
            method_result = self._run_diffedit(sample, inversion)
