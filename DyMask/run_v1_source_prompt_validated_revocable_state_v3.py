...
from DyMask.v1_source_prompt_validated_revocable_state_v3 import V1SourcePromptValidatedRevocableStateV3Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--outside-veto-strength", type=float, default=1.0)
    parser.add_argument("--disable-support-competition", action="store_true")
    parser.add_argument("--support-keep-ratio", type=float, default=0.20)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_prompt_validated_revocable_state_v3_runs/sp_vrs_v3"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_validated_revocable_state_v3"
        if config.phase != "custom"
        else "v1_source_prompt_validated_revocable_state_v3"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_prompt_validated_revocable_state_v3.md"))

    save_json(
        run_dir / "variant.json",
        {
            "variant_name": "source_prompt_validated_revocable_state_v3",
            ...
            "outside_veto_strength": float(args.outside_veto_strength),
            "support_competition_enabled": not args.disable_support_competition,
            "support_keep_ratio": float(args.support_keep_ratio),
            "mechanism": "Source-prompt dynamic mask with validated revocable state, zero-floor mask, outside veto, and support competition",
        },
    )
    ...
    logger.log(
        stage="source_prompt_validated_revocable_state_v3",
        ...
        conclusion="samples and validated-revocable-state-v3 config saved",
    ...
    editor = V1SourcePromptValidatedRevocableStateV3Editor(
        pipe,
        config,
        ...
        outside_veto_strength=args.outside_veto_strength,
        enable_support_competition=not args.disable_support_competition,
    ...
    logger.log(
        ...
        conclusion="validated-revocable-state-v3 run finished",
