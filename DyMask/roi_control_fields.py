...
@dataclass
class RoiTransformationFieldConfig:
    enabled: bool = True
...
@dataclass
class RoiPreservationFieldConfig:
    enabled: bool = True
    source: str = "hybrid"
    ...
    latent_drift_weight: float = 1.0

    def to_dict(self) -> dict[str, object]:
        return asdict(self)


class RoiControlFieldBuilder:
    """Build ROI-internal control fields without changing the fixed hard-ROI support policy.

    The transformation field T_t is implemented as a per-pixel edit-gain map.
    The preservation field P_t is implemented as a per-pixel source-anchor blend map.
    Both are zero-training controls intended for clean ablations.
    """

    def __init__(
        self,
        transformation: RoiTransformationFieldConfig,
        preservation: RoiPreservationFieldConfig,
    ) -> None:
        self.transformation = transformation
        self.preservation = preservation
        self._validate_source(self.transformation.source)
        self._validate_source(self.preservation.source)
    ...
    def build_transformation_field(
        ...
        field = float(config.min_scale) + (float(config.max_scale) - float(config.min_scale)) * activated
        return basis, field.to(device=dynamic_mask.device, dtype=dynamic_mask.dtype)
    ...
    def build_preservation_field(
        ...
        basis = self._smooth_map(basis, config.smoothing_kernel)
        if not config.enabled or not self._step_window_active(config.start_ratio, config.end_ratio, step_idx, total_steps):
            return basis, torch.zeros_like(dynamic_mask)

        activated = torch.sigmoid((basis - float(config.threshold)) * float(config.temperature))
        field = float(config.max_strength) * activated
        return basis, field.to(device=dynamic_mask.device, dtype=dynamic_mask.dtype)
