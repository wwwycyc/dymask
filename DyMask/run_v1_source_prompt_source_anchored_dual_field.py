...
from DyMask.v1_source_prompt_source_anchored_dual_field import (
    V1SourcePromptSourceAnchoredDualFieldEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--dual-field-mode",
        type=str,
        choices=("full", "transform_only", "preservation_only"),
        default="full",
    )
    ...
    run_prefix = f"{config.phase}_sp_dual_field" if config.phase != "custom" else "sp_dual_field"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_dual_field.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_dual_field_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI support with outside-roi source anchoring and roi-internal preservation/transformation dual fields",
            "dual_field_mode": args.dual_field_mode,
    ...
    logger.log(
        stage="source_prompt_source_anchored_dual_field",
        ...
        conclusion="samples and source-anchored dual-field config saved",
    ...
    editor = V1SourcePromptSourceAnchoredDualFieldEditor(
        pipe,
        config,
        dual_field_mode=args.dual_field_mode,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
    ...
    logger.log(
        ...
        conclusion="source-anchored dual-field run finished",
        next_step="compare against hard-roi baseline and dual-field ablations",
    )
