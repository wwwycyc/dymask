...
from DyMask.v1_source_prompt_source_anchored_support_c6b import V1SourcePromptSourceAnchoredSupportC6BEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c6b"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c6b"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c6b"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c6b.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": (
                "C6b keeps the C6 late roi-trust fallback and consistency floor, but tightens the roi-trust "
                "branch to regions where support_state and mask both agree. This is meant to reduce broad-roi "
                "over-relaxation while preserving late under-edit recovery."
            ),
            ...
            "background_anchor": (
                ...
                "R_t = alpha_t * roi_soft * max(sqrt(discrepancy * dynamic_mask), beta_t * sqrt(support_state * mask)) "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c6b",
        ...
        conclusion="samples and C6b source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC6BEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C6b source-anchored temporal-support prompt run finished",
        next_step="inspect whether agreement-style roi trust reduces broad-roi regressions relative to C6",
    )
