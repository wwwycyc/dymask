...
from DyMask.v1_source_prompt_source_anchored_support_c6d import V1SourcePromptSourceAnchoredSupportC6DEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c6d"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c6d"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c6d"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c6d.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": (
                "C6d keeps the C6 consistency floor and late fallback schedule, but further gates the C6c "
                "support-gap recovery branch with dynamic_mask. This is meant to refill live under-edited "
                "holes without letting stale support memory reopen regions where edit evidence has already "
                "disappeared."
            ),
            ...
            "background_anchor": (
                ...
                "R_t = alpha_t * roi_soft * max(sqrt(discrepancy * dynamic_mask), beta_t * sqrt(relu(support_state - mask) * dynamic_mask)) "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c6d",
        ...
        conclusion="samples and C6d source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC6DEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C6d source-anchored temporal-support prompt run finished",
        next_step="inspect whether dynamic-gated recovery improves under-edited holes without reopening stale regions relative to C6/C6b/C6c",
    )
