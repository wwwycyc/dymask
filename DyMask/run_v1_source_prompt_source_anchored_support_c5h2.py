...
from DyMask.v1_source_prompt_source_anchored_support_c5h2 import V1SourcePromptSourceAnchoredSupportC5H2Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5h2"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5h2"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5h2"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5h2.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": (
                "Generate DiffEdit soft ROI deterministically per sample, extract an adaptive hard core for support writes, "
                "then add only a normalized soft boundary residual for mask readout, source-anchor base, and confidence-gated relaxation."
            ),
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_core * dynamic_mask), S_0 = roi_core * dynamic_mask",
            "adaptive_mask": (
            ...
            "background_anchor": (
                ...
                "R_t = alpha_t * (roi_core + beta * roi_boundary) * sqrt(discrepancy * dynamic_mask) * (1 - |mask - dynamic_mask|); "
                "A'_t = lerp(A_t, 1, R_t); "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5h2",
        ...
        conclusion="samples and C5H2 deterministic-core-boundary config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5H2Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C5H2 deterministic-core-boundary run finished",
        next_step="inspect whether deterministic ROI plus adaptive core extraction recovers original C5 edit strength without returning to pseudo-hard behavior",
    )
