...
from DyMask.roi_control_fields import (
    FIELD_SOURCES,
...
from DyMask.v1_source_prompt_source_anchored_hard_roi_control_fields import (
    V1SourcePromptSourceAnchoredHardRoiControlFieldsEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--disable-transformation-field", action="store_true")
    parser.add_argument("--transform-source", choices=FIELD_SOURCES, default="dynamic_mask")
    parser.add_argument("--transform-start-ratio", type=float, default=0.15)
    parser.add_argument("--transform-end-ratio", type=float, default=0.85)
    parser.add_argument("--transform-threshold", type=float, default=0.50)
    parser.add_argument("--transform-temperature", type=float, default=8.0)
    parser.add_argument("--transform-smoothing-kernel", type=int, default=1)
    parser.add_argument("--transform-min-scale", type=float, default=0.85)
    parser.add_argument("--transform-max-scale", type=float, default=1.25)
    parser.add_argument("--transform-discrepancy-weight", type=float, default=1.0)
    parser.add_argument("--transform-attention-weight", type=float, default=0.5)
    parser.add_argument("--transform-latent-drift-weight", type=float, default=-0.5)

    parser.add_argument("--disable-preservation-field", action="store_true")
    parser.add_argument("--preserve-source", choices=FIELD_SOURCES, default="hybrid")
    parser.add_argument("--preserve-start-ratio", type=float, default=0.35)
    parser.add_argument("--preserve-end-ratio", type=float, default=1.0)
    parser.add_argument("--preserve-threshold", type=float, default=0.50)
    parser.add_argument("--preserve-temperature", type=float, default=8.0)
    parser.add_argument("--preserve-smoothing-kernel", type=int, default=3)
    parser.add_argument("--preserve-max-strength", type=float, default=0.35)
    parser.add_argument("--preserve-discrepancy-weight", type=float, default=-0.5)
    parser.add_argument("--preserve-attention-weight", type=float, default=-0.25)
    parser.add_argument("--preserve-latent-drift-weight", type=float, default=1.0)

    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_roi_field_runs/sp_anchor_roi_fields"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_anchor_roi_fields" if config.phase != "custom" else "sp_anchor_roi_fields"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi_control_fields.md"))
    ...
    transformation_config = RoiTransformationFieldConfig(
        enabled=not args.disable_transformation_field,
        source=str(args.transform_source),
        start_ratio=float(args.transform_start_ratio),
        end_ratio=float(args.transform_end_ratio),
        threshold=float(args.transform_threshold),
        temperature=float(args.transform_temperature),
        smoothing_kernel=int(args.transform_smoothing_kernel),
        min_scale=float(args.transform_min_scale),
        max_scale=float(args.transform_max_scale),
        discrepancy_weight=float(args.transform_discrepancy_weight),
        attention_weight=float(args.transform_attention_weight),
        latent_drift_weight=float(args.transform_latent_drift_weight),
    )
    preservation_config = RoiPreservationFieldConfig(
        enabled=not args.disable_preservation_field,
        source=str(args.preserve_source),
        start_ratio=float(args.preserve_start_ratio),
        end_ratio=float(args.preserve_end_ratio),
        threshold=float(args.preserve_threshold),
        temperature=float(args.preserve_temperature),
        smoothing_kernel=int(args.preserve_smoothing_kernel),
        max_strength=float(args.preserve_max_strength),
        discrepancy_weight=float(args.preserve_discrepancy_weight),
        attention_weight=float(args.preserve_attention_weight),
        latent_drift_weight=float(args.preserve_latent_drift_weight),
    )

    save_json(
        run_dir / "variant.json",
        {
            "variant_name": "source_prompt_source_anchored_hard_roi_control_fields_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI editing with outside-roi source anchoring plus ROI-local transformation and preservation fields",
            "support_update": "M_t = roi_mask",
            "background_anchor": "z_{t-1} = roi_mask * z_{t-1}^{edit} + (1-roi_mask) * z_{t-1}^{src}",
            "transformation_field": transformation_config.to_dict(),
            "preservation_field": preservation_config.to_dict(),
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi_control_fields",
        ...
        conclusion="samples and ROI control-field config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiControlFieldsEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored hard-roi ROI-control-field run finished",
        next_step="compare T_t and P_t ablations against hard-roi and feature-injection baselines",
    )
