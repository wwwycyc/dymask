...
from DyMask.adaptive_roi_residual_gain import FIELD_SOURCES, CHANNEL_SOURCES, AdaptiveRoiResidualGainConfig
...
from DyMask.v1_source_prompt_source_anchored_hard_roi_adaptive_residual_gain import (
    V1SourcePromptSourceAnchoredHardRoiAdaptiveResidualGainEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--disable-arrg", action="store_true")
    parser.add_argument("--disable-arrg-spatial", action="store_true")
    parser.add_argument("--disable-arrg-channel", action="store_true")
    parser.add_argument("--enable-arrg-core-shell", action="store_true")
    parser.add_argument("--arrg-core-erosion-kernel", type=int, default=3)
    parser.add_argument("--arrg-shell-scale", type=float, default=1.0)
    parser.add_argument("--enable-arrg-temporal-basis", action="store_true")
    parser.add_argument("--arrg-basis-rho", type=float, default=0.85)
    parser.add_argument("--enable-arrg-temporal-channel", action="store_true")
    parser.add_argument("--arrg-channel-rho", type=float, default=0.85)
    parser.add_argument("--arrg-source", choices=FIELD_SOURCES, default="hybrid")
    parser.add_argument("--arrg-start-ratio", type=float, default=0.20)
    parser.add_argument("--arrg-end-ratio", type=float, default=0.90)
    parser.add_argument("--arrg-temporal-power", type=float, default=1.5)
    parser.add_argument("--arrg-threshold", type=float, default=0.45)
    parser.add_argument("--arrg-temperature", type=float, default=8.0)
    parser.add_argument("--arrg-smoothing-kernel", type=int, default=3)
    parser.add_argument("--arrg-min-scale", type=float, default=1.0)
    parser.add_argument("--arrg-max-scale", type=float, default=1.45)
    parser.add_argument("--arrg-discrepancy-weight", type=float, default=1.0)
    parser.add_argument("--arrg-attention-weight", type=float, default=0.75)
    parser.add_argument("--arrg-latent-drift-weight", type=float, default=-0.25)
    parser.add_argument("--arrg-channel-source", choices=CHANNEL_SOURCES, default="residual_abs")
    parser.add_argument("--arrg-channel-strength", type=float, default=0.20)
    parser.add_argument("--arrg-channel-temperature", type=float, default=6.0)

    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_adaptive_roi_residual_gain_runs/sp_anchor_arrg"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_anchor_arrg" if config.phase != "custom" else "sp_anchor_arrg"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi_adaptive_residual_gain.md"))
    ...
    gain_config = AdaptiveRoiResidualGainConfig(
        enabled=not args.disable_arrg,
        enable_spatial_gain=not args.disable_arrg_spatial,
        enable_channel_gain=not args.disable_arrg_channel,
        enable_core_shell=bool(args.enable_arrg_core_shell),
        core_erosion_kernel=int(args.arrg_core_erosion_kernel),
        shell_scale=float(args.arrg_shell_scale),
        enable_temporal_basis=bool(args.enable_arrg_temporal_basis),
        basis_rho=float(args.arrg_basis_rho),
        enable_temporal_channel=bool(args.enable_arrg_temporal_channel),
        channel_rho=float(args.arrg_channel_rho),
        source=str(args.arrg_source),
        start_ratio=float(args.arrg_start_ratio),
        end_ratio=float(args.arrg_end_ratio),
        temporal_power=float(args.arrg_temporal_power),
        threshold=float(args.arrg_threshold),
        temperature=float(args.arrg_temperature),
        smoothing_kernel=int(args.arrg_smoothing_kernel),
        min_scale=float(args.arrg_min_scale),
        max_scale=float(args.arrg_max_scale),
        discrepancy_weight=float(args.arrg_discrepancy_weight),
        attention_weight=float(args.arrg_attention_weight),
        latent_drift_weight=float(args.arrg_latent_drift_weight),
        channel_source=str(args.arrg_channel_source),
        channel_strength=float(args.arrg_channel_strength),
        channel_temperature=float(args.arrg_channel_temperature),
    )
    ...
    save_json(
        ...
        {
            "variant_name": (
                "source_prompt_source_anchored_hard_roi_adaptive_residual_gain_v2"
                if (gain_config.enable_temporal_basis or gain_config.enable_temporal_channel or gain_config.enable_core_shell)
                else "source_prompt_source_anchored_hard_roi_adaptive_residual_gain_v1"
            ),
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": (
                "Hard ROI editing with outside-roi source anchoring and adaptive ROI-local residual gain "
                "with spatial and channel modulation"
            ),
            "support_update": "M_t = roi_mask",
            "background_anchor": "z_{t-1} = roi_mask * z_{t-1}^{edit} + (1-roi_mask) * z_{t-1}^{src}",
            "adaptive_roi_residual_gain": gain_config.to_dict(),
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi_adaptive_residual_gain",
        ...
        conclusion="samples and adaptive residual gain config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiAdaptiveResidualGainEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="adaptive residual gain run finished",
        next_step="inspect summary tables and ROI-internal edit strength",
    )
