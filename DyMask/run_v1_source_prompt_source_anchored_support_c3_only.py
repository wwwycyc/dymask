...
from DyMask.v1_source_prompt_source_anchored_support_c3_only import V1SourcePromptSourceAnchoredSupportC3OnlyEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c3_only"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c3_only"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c3_only"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c3_only.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c3_only_delayed_soft_support_v1",
            ...
            "mechanism": "Baseline adaptive temporal support plus delayed soft-support injection only",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c3_only",
        ...
        conclusion="samples and C3-only source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC3OnlyEditor(
        pipe,
        config,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_source_anchored_support_c3_only_delayed_soft_support_v1",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="C3-only source-anchored temporal-support prompt run finished",
        next_step="inspect whether delayed soft-support alone reduces under-editing without changing anchoring",
    )
