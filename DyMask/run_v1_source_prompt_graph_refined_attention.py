...
from DyMask.v1_source_prompt_graph_refined_attention import V1SourcePromptGraphRefinedAttentionEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_graph_attention_runs/dymask_v1_source_prompt_graph_refined_attention"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_graph_refined_attention"
        if config.phase != "custom"
        else "v1_source_prompt_graph_refined_attention"
    )
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_graph_refined_attention_v1",
            ...
            "mechanism": "Hard ROI bounded temporal support with self-attention-guided graph refinement of A_t",
    ...
    logger.log(
        stage="source_prompt_graph_refined_attention",
        ...
        conclusion="samples and graph-refined-attention variant config saved",
    ...
    editor = V1SourcePromptGraphRefinedAttentionEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="graph-refined-attention prompt run finished",
        next_step="inspect summary tables and refined-attention diagnostics",
    )
