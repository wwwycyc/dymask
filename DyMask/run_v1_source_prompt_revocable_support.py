...
from DyMask.v1_source_prompt_revocable_support import V1SourcePromptRevocableSupportEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_revocable_runs/dymask_v1_source_prompt_revocable_support"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_revocable_support"
        if config.phase != "custom"
        else "v1_source_prompt_revocable_support"
    )
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_revocable_support_v1",
            ...
            "mechanism": "Hard ROI bounded revocable temporal support accumulation over dynamic evidence",
            "support_update": (
    ...
    logger.log(
        stage="source_prompt_revocable_support",
        ...
        conclusion="samples and revocable-support variant config saved",
    ...
    editor = V1SourcePromptRevocableSupportEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="revocable-support prompt run finished",
        next_step="inspect summary tables and revocable support diagnostics",
    )
