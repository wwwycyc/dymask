...
from DyMask.v1_source_prompt_source_anchored_support_c5b import V1SourcePromptSourceAnchoredSupportC5BEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--mask-boundary-soft-scale", type=float, default=1.0)
    parser.add_argument("--anchor-boundary-soft-scale", type=float, default=1.0)
    parser.add_argument("--confidence-boundary-soft-scale", type=float, default=1.0)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5b"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5b"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5b"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5b.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c5b_split_roi_roles_v1",
            ...
            "mask_boundary_soft_scale": float(args.mask_boundary_soft_scale),
            "anchor_boundary_soft_scale": float(args.anchor_boundary_soft_scale),
            "confidence_boundary_soft_scale": float(args.confidence_boundary_soft_scale),
            "mechanism": (
                "Split the roi roles. Hard roi controls interior support writes and interior edit permission. "
                "True soft roi only contributes a dynamic boundary tail outside the hard roi. "
                "Anchor confidence uses a separate spatial gate so interior relaxation is no longer damped by soft-roi amplitude."
            ),
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_hard * dynamic_mask), S_0 = roi_hard * dynamic_mask",
            "adaptive_mask": (
                "M_t = roi_hard * S_t + beta_m * w_t * roi_soft * (1-roi_hard) * dynamic_mask, "
                "w_t follows cosine decay from soft_roi_start_weight to soft_roi_end_weight"
            ),
            "background_anchor": (
                "A_t = roi_hard + beta_a * (1-h_t) * roi_soft * (1-roi_hard); "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5b",
        ...
        conclusion="samples and C5B split-roi config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5BEditor(
        pipe,
        config,
        ...
        mask_boundary_soft_scale=args.mask_boundary_soft_scale,
        anchor_boundary_soft_scale=args.anchor_boundary_soft_scale,
        confidence_boundary_soft_scale=args.confidence_boundary_soft_scale,
    ...
    logger.log(
        ...
        conclusion="C5B split-roi source-anchored temporal-support prompt run finished",
        next_step="inspect whether separating interior hard-roi control from boundary soft-roi smoothing recovers edit strength",
    )
