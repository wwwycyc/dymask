...
from DyMask.v1_source_prompt_source_anchored_hard_roi_proedit_like import (
    V1SourcePromptSourceAnchoredHardRoiProEditLikeEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_proedit_like_runs/sp_anchor_proedit_like"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_sp_anchor_proedit_like"
        if config.phase != "custom"
        else "sp_anchor_proedit_like"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi_proedit_like.md"))
    ...
    if not 0.0 <= float(args.mix_inside_target_relax_strength) <= 1.0:
        raise ValueError("mix-inside-target-relax-strength must be within [0, 1]")
    if not 0.0 <= float(args.mix_outside_source_strength) <= 1.0:
        raise ValueError("mix-outside-source-strength must be within [0, 1]")
    if not 0.0 <= float(args.latent_shift_strength) <= 1.0:
        raise ValueError("latent-shift-strength must be within [0, 1]")
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_hard_roi_proedit_like_v2",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI editing with outside-roi source anchoring and ProEdit-inspired localized latent/KV mixing",
            ...
            "proedit_like": {
                ...
                "latent_shift": {
                    "strength": latent_shift_config.strength,
                },
            },
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi_proedit_like",
        ...
        conclusion="samples and ProEdit-inspired KV-mix config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiProEditLikeEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored hard-roi ProEdit-inspired run finished",
        next_step="compare against hard-roi baseline and inspect edit-vs-preservation tradeoff",
    )
    editor.close()
