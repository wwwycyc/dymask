...
from DyMask.v1_source_prompt_graph_refined_revocable_support import V1SourcePromptGraphRefinedRevocableSupportEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--attention-graph-beta", type=float, default=0.60)
    parser.add_argument("--attention-graph-steps", type=int, default=2)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_combo_runs/sp_gr_rev"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_sp_gr_rev"
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_graph_refined_revocable_support_v1",
            ...
            "attention_graph_beta": float(args.attention_graph_beta),
            "attention_graph_steps": int(args.attention_graph_steps),
            "mechanism": "Hard ROI bounded revocable temporal support with self-attention-guided graph refinement of A_t",
            "attention_update": "A_t = GraphRefine(A_t_raw, W_t^self)",
            "support_update": (
                "Q_t = mu * Q_{t-1} + (1-mu) * (1-phi_t), "
                "S_t = rho * S_{t-1} + (1-rho) * phi_t - lambda * Q_t * S_{t-1}"
            ),
    ...
    logger.log(
        stage="source_prompt_graph_refined_revocable_support",
        ...
        conclusion="samples and graph-refined-revocable variant config saved",
    ...
    editor = V1SourcePromptGraphRefinedRevocableSupportEditor(
        pipe,
        config,
        support_rho=args.support_rho,
        support_decay_mu=args.support_decay_mu,
        support_decay_lambda=args.support_decay_lambda,
        attention_graph_beta=args.attention_graph_beta,
        attention_graph_steps=args.attention_graph_steps,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_graph_refined_revocable_support_v1",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="graph-refined-revocable prompt run finished",
        next_step="inspect summary tables and merged attention/support diagnostics",
    )
