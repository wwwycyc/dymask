...
from DyMask.v1_source_prompt_source_anchored_validated_support_factorized import (
    V1SourcePromptSourceAnchoredValidatedSupportFactorizedEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_factorized_runs/sp_factor_anchor"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_factor_anchor" if config.phase != "custom" else "sp_factor_anchor"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_factorized_support.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_validated_support_factorized_v1",
            ...
            "mechanism": "Factorized validated discrepancy accumulation with outside-roi source latent anchoring",
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (Norm(D_t) * Pi_t * C_t^cred)",
    ...
    logger.log(
        stage="source_prompt_source_anchored_validated_support_factorized",
        ...
        conclusion="samples and source-anchored factorized-support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredValidatedSupportFactorizedEditor(
        pipe,
        config,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_source_anchored_validated_support_factorized_v1",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="source-anchored factorized support run finished",
