...
from DyMask.v1_source_prompt_validated_support_factorized import V1SourcePromptValidatedSupportFactorizedEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "runs/dymask_v1_source_prompt_validated_support_factorized"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_validated_support_factorized"
        if config.phase != "custom"
        else "v1_source_prompt_validated_support_factorized"
    )
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_validated_support_factorized_v1",
            ...
            "mechanism": "Support-validated discrepancy accumulation with explicit credibility factor",
    ...
    logger.log(
        stage="source_prompt_validated_support_factorized",
        ...
        conclusion="samples and factorized validated-support variant config saved",
    ...
    editor = V1SourcePromptValidatedSupportFactorizedEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="factorized validated-support prompt run finished",
        next_step="inspect summary tables and credibility-aware validation diagnostics",
    )
