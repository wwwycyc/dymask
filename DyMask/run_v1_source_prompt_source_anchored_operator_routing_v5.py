...
from DyMask.v1_source_prompt_source_anchored_operator_routing_v5 import (
    V1SourcePromptSourceAnchoredOperatorRoutingV5Editor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--interiority-bias", type=float, default=1.0)
    parser.add_argument("--boundary-rewrite-bias", type=float, default=1.0)
    parser.add_argument("--boundary-stabilize-bias", type=float, default=1.0)
    parser.add_argument("--relative-update-power", type=float, default=1.0)
    parser.add_argument("--trajectory-consistency-power", type=float, default=1.0)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_operator_routing_v5_runs/sp_operator_routing_v5"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_operator_routing_v5" if config.phase != "custom" else "sp_operator_routing_v5"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_operator_routing_v5.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_operator_routing_v5",
            ...
            "interiority_bias": args.interiority_bias,
            "boundary_rewrite_bias": args.boundary_rewrite_bias,
            "boundary_stabilize_bias": args.boundary_stabilize_bias,
            "relative_update_power": args.relative_update_power,
            "trajectory_consistency_power": args.trajectory_consistency_power,
    ...
    logger.log(
        stage="source_prompt_source_anchored_operator_routing_v5",
        ...
        conclusion="samples and source-anchored operator-routing-v5 config saved",
    ...
    editor = V1SourcePromptSourceAnchoredOperatorRoutingV5Editor(
        pipe,
        config,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
        transform_attention_weight=args.transform_attention_weight,
    ...
    logger.log(
        ...
        conclusion="source-anchored operator-routing-v5 run finished",
        next_step="compare against hard-roi baseline and inspect mixed-task visuals",
    )
