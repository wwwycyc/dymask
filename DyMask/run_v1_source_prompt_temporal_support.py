...
from DyMask.v1_source_prompt_temporal_support import V1SourcePromptTemporalSupportEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "runs/dymask_v1_source_prompt_temporal_support"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_temporal_support"
        if config.phase != "custom"
        else "v1_source_prompt_temporal_support"
    )
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_temporal_support_v1",
            ...
            "mechanism": "Hard ROI bounded temporal support accumulation over dynamic evidence",
    ...
    logger.log(
        stage="source_prompt_temporal_support",
        ...
        conclusion="samples and temporal-support variant config saved",
    ...
    editor = V1SourcePromptTemporalSupportEditor(
        pipe,
        config,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_temporal_support_v1",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="temporal-support prompt run finished",
