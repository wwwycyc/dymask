...
from DyMask.v1_source_prompt_source_anchored_support_c2 import V1SourcePromptSourceAnchoredSupportC2Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c2"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c2"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c2"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c2.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": "Adaptive temporal support with soft-aware support memory and delayed source anchoring that ramps in after early editing has formed",
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (dynamic_mask * lerp(roi_hard, roi_soft, beta)), S_0 = dynamic_mask * lerp(roi_hard, roi_soft, beta)",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c2",
        ...
        conclusion="samples and C2 source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC2Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C2 source-anchored temporal-support prompt run finished",
        next_step="inspect summary tables and delayed-anchor behavior",
    )
