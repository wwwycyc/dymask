...
from DyMask.v1_source_prompt_source_anchored_dual_latent_blend import (
    V1SourcePromptSourceAnchoredDualLatentBlendEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_dual_blend_runs/sp_dual_blend"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_dual_blend" if config.phase != "custom" else "sp_dual_blend"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_dual_latent_blend.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_dual_latent_blend_v1",
            ...
            "mechanism": "Hard ROI bounded temporal support with source-target dual latent recomposition",
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_mask * dynamic_mask), S_0 = roi_mask * dynamic_mask",
            "step_update": "z_{t-1} = m_t * z_{t-1}^{tar} + (roi-m_t) * z_{t-1}^{src-pred} + (1-roi) * z_{t-1}^{src}",
    ...
    logger.log(
        stage="source_prompt_source_anchored_dual_latent_blend",
        ...
        conclusion="samples and dual-latent-blend config saved",
    ...
    editor = V1SourcePromptSourceAnchoredDualLatentBlendEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored dual-latent-blend run finished",
