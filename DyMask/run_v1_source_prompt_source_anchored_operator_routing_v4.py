...
from DyMask.v1_source_prompt_source_anchored_operator_routing_v4 import (
    V1SourcePromptSourceAnchoredOperatorRoutingV4Editor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_operator_routing_v4_runs/sp_operator_routing_v4"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_operator_routing_v4" if config.phase != "custom" else "sp_operator_routing_v4"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_operator_routing_v4.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_operator_routing_v4",
    ...
    logger.log(
        stage="source_prompt_source_anchored_operator_routing_v4",
        ...
        conclusion="samples and source-anchored operator-routing-v4 config saved",
    ...
    editor = V1SourcePromptSourceAnchoredOperatorRoutingV4Editor(
        pipe,
        config,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
        transform_attention_weight=args.transform_attention_weight,
    ...
    logger.log(
        ...
        conclusion="source-anchored operator-routing-v4 run finished",
