...
from DyMask.v1_source_prompt_source_anchored_support_c1 import V1SourcePromptSourceAnchoredSupportC1Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c1"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c1"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c1"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c1.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": "Adaptive temporal support with soft-aware support memory and unchanged scheduled outside-roi source latent anchoring",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c1",
        ...
        conclusion="samples and C1 source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC1Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C1 source-anchored temporal-support prompt run finished",
        next_step="inspect summary tables and visual under-editing changes",
    )
