...
from DyMask.v1_source_prompt_source_anchored_support_c5d import V1SourcePromptSourceAnchoredSupportC5DEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--control-soft-roi-quantile", type=float, default=0.90)
    parser.add_argument("--control-soft-roi-gamma", type=float, default=1.0)
    parser.add_argument("--adaptive-target-p90", type=float, default=0.75)
    parser.add_argument("--adaptive-trigger-p90", type=float, default=0.55)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5d"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5d"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5d"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5d.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c5d_adaptive_control_soft_roi_v1",
            ...
            "control_soft_roi_quantile": float(args.control_soft_roi_quantile),
            "control_soft_roi_gamma": float(args.control_soft_roi_gamma),
            "adaptive_target_p90": float(args.adaptive_target_p90),
            "adaptive_trigger_p90": float(args.adaptive_trigger_p90),
            "mechanism": (
                "Use true soft DiffEdit ROI by default, but only partially rescue it toward the rescaled control map "
                "when the per-sample soft-roi high quantile is too weak."
            ),
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_mask * dynamic_mask), S_0 = roi_mask * dynamic_mask",
            "adaptive_mask": (
                "M_t = (1-w_t) * S_t + w_t * roi_soft_control_adaptive, "
                "where roi_soft_control_adaptive = lerp(roi_soft, roi_soft_rescaled, boost_strength)"
            ),
            "adaptive_boost": (
                "boost_strength = clamp((target_p90 - q90) / (target_p90 - trigger_p90), 0, 1), "
                "q90 from the true soft-roi map"
            ),
            "background_anchor": (
                "A_t = lerp(roi_soft_control_adaptive, roi_hard, h_t); "
                "R_t = alpha_t * roi_soft_control_adaptive * sqrt(discrepancy * dynamic_mask) * (1 - |mask - dynamic_mask|); "
                "A'_t = lerp(A_t, 1, R_t); "
                "z_{t-1} = A'_t * z_{t-1}^{edit} + (1-A'_t) * z_{t-1}^{src}"
            ),
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5d",
        ...
        conclusion="samples and C5D adaptive-control-soft-roi config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5DEditor(
        pipe,
        config,
        ...
        control_soft_roi_quantile=args.control_soft_roi_quantile,
        control_soft_roi_gamma=args.control_soft_roi_gamma,
        adaptive_target_p90=args.adaptive_target_p90,
        adaptive_trigger_p90=args.adaptive_trigger_p90,
    ...
    logger.log(
        ...
        conclusion="C5D adaptive-control-soft-roi rescue run finished",
        next_step="inspect whether adaptive rescue keeps most of the edit gain while avoiding global background drift",
    )
