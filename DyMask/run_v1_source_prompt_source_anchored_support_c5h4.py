...
from DyMask.v1_source_prompt_source_anchored_support_c5h4 import V1SourcePromptSourceAnchoredSupportC5H4Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5h4"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5h4"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5h4"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5h4.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "underedit_temporal_rho": float(args.underedit_temporal_rho),
            "mechanism": (
                "Keep the deterministic C5H2 core-boundary support path, compute the same raw under-edit pressure map "
                "r_t = roi_soft * dynamic_mask * relu(g_src_tar - g_applied)/(g_src_tar + eps), then pass it through a temporal "
                "persistence state u_t = gamma * u_(t-1) + (1-gamma) * r_t and only apply the confirmed rescue r\'_t = min(r_t, u_t)."
            ),
            "underedit_temporal_guard": (
                "u_t = gamma * u_(t-1) + (1-gamma) * r_t; r\'_t = min(r_t, u_t)"
            ),
            "support_update": (
            ...
            "adaptive_mask": (
                "M'_t = clamp(M_t + lambda_m * r\'_t, 0, 1), where M_t keeps the C5H2 core-boundary readout"
            ),
            "background_anchor": (
                "A''_t = lerp(A'_t, 1, lambda_a * r\'_t), where A'_t is the C5H2 confidence-relaxed anchor mask"
            ),
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5h4",
        ...
        conclusion="samples and C5H4 temporal under-edit guard config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5H4Editor(
        pipe,
        config,
        ...
        underedit_temporal_rho=args.underedit_temporal_rho,
    ...
    logger.log(
        ...
        conclusion="C5H4 temporal under-edit guard run finished",
        next_step=(
            "inspect whether the temporally-confirmed under-edit rescue improves the weak-edit cases without reintroducing instability"
        ),
    )
