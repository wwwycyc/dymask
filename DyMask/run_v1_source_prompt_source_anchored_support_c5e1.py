...
from DyMask.v1_source_prompt_source_anchored_support_c5e1 import V1SourcePromptSourceAnchoredSupportC5E1Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--confidence-deficit-rescue-strength", type=float, default=0.30)
    parser.add_argument("--confidence-deficit-gap-power", type=float, default=1.0)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5e1"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5e1"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5e1"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5e1.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c5e1_deficit_confidence_rescue_v1",
            ...
            "confidence_deficit_rescue_strength": float(args.confidence_deficit_rescue_strength),
            "confidence_deficit_gap_power": float(args.confidence_deficit_gap_power),
            "mechanism": (
                "Keep true-soft C5 readout and baseline adaptive anchor schedule unchanged. "
                "Only add local confidence rescue where dynamic_mask is stronger than the current effective mask."
            ),
            ...
            "background_anchor": (
                ...
                "gap = relu(dynamic_mask - mask); "
                "rescue = roi_soft * sqrt(discrepancy * dynamic_mask) * gap^p; "
                "confidence' = clamp(roi_soft * sqrt(discrepancy * dynamic_mask) * (1 - |mask - dynamic_mask|) + lambda * rescue, 0, 1); "
                "R_t = alpha_t * confidence'; "
                "A'_t = lerp(A_t, 1, R_t); "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5e1",
        ...
        conclusion="samples and C5E1 deficit-confidence-rescue config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5E1Editor(
        pipe,
        config,
        ...
        confidence_deficit_rescue_strength=args.confidence_deficit_rescue_strength,
        confidence_deficit_gap_power=args.confidence_deficit_gap_power,
    ...
    logger.log(
        ...
        conclusion="C5E1 deficit-confidence-rescue run finished",
        next_step="inspect whether deficit-triggered confidence rescue recovers edit strength without broadening the control map",
    )
