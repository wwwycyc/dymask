...
from DyMask.v1_source_prompt_source_anchored_soft_roi import V1SourcePromptSourceAnchoredSoftRoiEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_soft_roi_runs/sp_anchor_soft_roi"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_anchor_soft_roi" if config.phase != "custom" else "sp_anchor_soft_roi"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_soft_roi.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_soft_roi_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Soft DiffEdit ROI update with hard outside-roi source latent anchoring",
            "support_update": "M_t = roi_mask_soft",
            "background_anchor": "z_{t-1} = roi_mask_hard * z_{t-1}^{edit} + (1-roi_mask_hard) * z_{t-1}^{src}",
    ...
    logger.log(
        stage="source_prompt_source_anchored_soft_roi",
        ...
        conclusion="samples and source-anchored soft-roi config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSoftRoiEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored soft-roi run finished",
        next_step="compare against hard-roi and DiffEdit",
    )
