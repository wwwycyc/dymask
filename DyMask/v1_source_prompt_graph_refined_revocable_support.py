...
class V1SourcePromptGraphRefinedRevocableSupportEditor(V1SourcePromptHardRoiLockedEditor):
    def __init__(
        self,
        pipe,
        config,
        ...
        payload.update(
            {
                ...
                "roi_mask_policy": "always hard roi times dynamic evidence with graph-refined attention and revocable support accumulation",
    ...
    def _initialize_support_states(
        self,
    ...
    def _update_support_states(
        self,
        previous_state: torch.Tensor | None,
        previous_low_evidence_state: torch.Tensor | None,
        evidence: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        low_evidence_state = (
            self.support_decay_mu * previous_low_evidence_state
            + (1.0 - self.support_decay_mu) * (1.0 - evidence)
        )
        support_state = (
            self.support_rho * previous_state
            + (1.0 - self.support_rho) * evidence
            - self.support_decay_lambda * low_evidence_state * previous_state
        )
    ...
    def _probe_method_batch_memory(
        ...
        try:
            ...
            if method_name == "target_only":
                ...
                effective_mask, _low_evidence_state = self._update_support_states(
                    None,
                    None,
                    support_evidence,
                )
    ...
    def _run_method_batch(
        ...
        low_evidence_state: torch.Tensor | None = None
        ...
        for step_idx, timestep in enumerate(self.pipe.scheduler.timesteps):
            ...
            if method_name == "target_only":
                ...
                support_state, low_evidence_state = self._initialize_support_states(dynamic_mask)
                ...
                support_state, low_evidence_state = self._update_support_states(
                    support_state,
                    low_evidence_state,
                    support_evidence,
                )
