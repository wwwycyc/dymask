...
from DyMask.v1_source_prompt_source_anchored_operator_routing_v9 import (
    V1SourcePromptSourceAnchoredOperatorRoutingV9Editor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--core-rewrite-max-gain", type=float, default=0.75)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_operator_routing_v9_runs/sp_operator_routing_v9"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_operator_routing_v9" if config.phase != "custom" else "sp_operator_routing_v9"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_operator_routing_v9.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_operator_routing_v9",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI support with outside-roi source anchoring, core-only rewrite gain, and boundary residual stabilization",
            ...
            "core_rewrite_max_gain": args.core_rewrite_max_gain,
    ...
    logger.log(
        stage="source_prompt_source_anchored_operator_routing_v9",
        ...
        conclusion="samples and source-anchored operator-routing-v9 config saved",
    ...
    editor = V1SourcePromptSourceAnchoredOperatorRoutingV9Editor(
        pipe,
        config,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
        transform_attention_weight=args.transform_attention_weight,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_source_anchored_operator_routing_v9",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="source-anchored operator-routing-v9 run finished",
