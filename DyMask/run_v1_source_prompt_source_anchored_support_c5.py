...
from DyMask.v1_source_prompt_source_anchored_support_c5 import V1SourcePromptSourceAnchoredSupportC5Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": (
                ...
                "Support memory stays baseline; source anchoring is relaxed only where roi, discrepancy, "
                "and dynamic support agree."
            ),
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5",
        ...
        conclusion="samples and C5 source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C5 source-anchored temporal-support prompt run finished",
        next_step="inspect whether local confidence-gated anchor relaxation improves edit strength without c2-style drift",
    )
