...
from DyMask.v1_source_prompt_source_anchored_support_c5h1 import V1SourcePromptSourceAnchoredSupportC5H1Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5h1"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5h1"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5h1"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5h1.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "soft_boundary_start_weight": float(args.soft_boundary_start_weight),
            "soft_boundary_end_weight": float(args.soft_boundary_end_weight),
            "mechanism": (
                "Keep H0's hard-core temporal support, then add only a thin soft boundary outside the hard ROI "
                "for readout and source-anchor base before confidence-gated relaxation."
            ),
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_hard * dynamic_mask), S_0 = roi_hard * dynamic_mask",
            "adaptive_mask": "M_t = clamp(S_t + b_t * relu(roi_soft - roi_hard), 0, 1)",
            "background_anchor": (
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5h1",
        ...
        conclusion="samples and C5H1 hard-core-soft-boundary config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5H1Editor(
        pipe,
        config,
        ...
        soft_boundary_start_weight=args.soft_boundary_start_weight,
        soft_boundary_end_weight=args.soft_boundary_end_weight,
    ...
    logger.log(
        ...
        conclusion="C5H1 hard-core-soft-boundary run finished",
        next_step="inspect whether a thin soft boundary recovers edge quality without giving away H0's edit strength",
    )
