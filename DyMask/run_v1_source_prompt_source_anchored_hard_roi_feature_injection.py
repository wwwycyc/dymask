...
from DyMask.v1_source_prompt_source_anchored_hard_roi_feature_injection import (
    V1SourcePromptSourceAnchoredHardRoiFeatureInjectionEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--inject-strength", type=float, default=0.5)
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_feature_inject_runs/sp_anchor_feat_inject"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_sp_anchor_feat_inject"
        if config.phase != "custom"
        else "sp_anchor_feat_inject"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi_feature_injection.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_hard_roi_feature_injection_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI editing with outside-roi source anchoring and decoder feature injection",
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi_feature_injection",
        ...
        conclusion="samples and decoder-feature-injection config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiFeatureInjectionEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored hard-roi feature-injection run finished",
        next_step="compare against hard-roi baseline and inspect visual edit strength",
    )
