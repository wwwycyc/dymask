...
from DyMask.v1_source_prompt_source_anchored_support_c5c import V1SourcePromptSourceAnchoredSupportC5CEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5c"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5c"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5c"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5c.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c5c_control_soft_roi_v1",
            ...
            "mechanism": (
                "Rescue true-soft C5 by rescaling the soft DiffEdit ROI before it is used for mask readout, "
                "anchor scheduling, and confidence gating, while keeping the original soft ROI for geometry and diagnostics."
            ),
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_mask * dynamic_mask), S_0 = roi_mask * dynamic_mask",
            "adaptive_mask": (
                "M_t = (1-w_t) * S_t + w_t * roi_soft_control, "
                "where roi_soft_control = max(roi_hard, clamp((roi_soft / q)^gamma, 0, 1))"
            ),
            "background_anchor": (
                "A_t = lerp(roi_soft_control, roi_hard, h_t); "
                "R_t = alpha_t * roi_soft_control * sqrt(discrepancy * dynamic_mask) * (1 - |mask - dynamic_mask|); "
                "A'_t = lerp(A_t, 1, R_t); "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5c",
        ...
        conclusion="samples and C5C control-soft-roi config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5CEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C5C control-soft-roi rescue run finished",
        next_step="inspect whether control-soft-roi rescaling restores edit strength without losing locality",
    )
