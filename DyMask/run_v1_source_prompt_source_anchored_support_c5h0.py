...
from DyMask.v1_source_prompt_source_anchored_support_c5h0 import V1SourcePromptSourceAnchoredSupportC5H0Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5h0"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5h0"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5h0"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5h0.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "mechanism": (
                "Keep C5 temporal support and confidence-gated anchor relaxation, but force the readout and "
                "anchor base to stay on the hard DiffEdit ROI."
            ),
            "support_update": "S_t = rho * S_{t-1} + (1-rho) * (roi_hard * dynamic_mask), S_0 = roi_hard * dynamic_mask",
            "adaptive_mask": "M_t = S_t",
            "background_anchor": (
                "A_t = roi_hard; "
                "R_t = alpha_t * roi_hard * sqrt(discrepancy * dynamic_mask) * (1 - |mask - dynamic_mask|); "
                "A'_t = lerp(A_t, 1, R_t); "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5h0",
        ...
        conclusion="samples and C5H0 hard-core config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5H0Editor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C5H0 hard-core run finished",
        next_step="inspect whether forcing hard-core readout restores edit strength without losing C5's anchor stability",
    )
