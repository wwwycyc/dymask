...
from DyMask.v1_source_prompt_source_anchored_support_c5a import V1SourcePromptSourceAnchoredSupportC5AEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument('--support-boundary-soft-scale', type=float, default=0.35)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = 'scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5a'

    config = build_config(args)
    run_prefix = (
        f'{config.phase}_source_prompt_source_anchored_support_c5a'
        if config.phase != 'custom'
        else 'v1_source_prompt_source_anchored_support_c5a'
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path('log_source_anchored_support_c5a.md'))
    ...
    save_json(
        ...
        {
            'variant_name': 'source_prompt_source_anchored_support_c5a_boundary_soft_write_v1',
            ...
            'support_boundary_soft_scale': float(args.support_boundary_soft_scale),
            'mechanism': (
                'Corrected C5 plus boundary-only soft support writes. Support memory still stays roi-bounded, '
                'but can absorb a limited soft fringe outside the hard roi while preserving hard-roi interior.'
            ),
            'support_update': (
                'S_t = rho * S_{t-1} + (1-rho) * (write_roi * dynamic_mask), '
                'write_roi = roi_hard + beta * roi_soft * (1-roi_hard)'
            ),
            ...
            'background_anchor': (
                ...
                'R_t = alpha_t * roi_soft * sqrt(discrepancy * dynamic_mask) * (1 - |mask - dynamic_mask|); '
                "A'_t = lerp(A_t, 1, R_t); "
    ...
    logger.log(
        stage='source_prompt_source_anchored_support_c5a',
        ...
        conclusion='samples and C5A source-anchored support config saved',
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5AEditor(
        pipe,
        config,
        ...
        support_boundary_soft_scale=args.support_boundary_soft_scale,
    ...
    logger.log(
        ...
        conclusion='C5A source-anchored temporal-support prompt run finished',
        next_step='inspect whether boundary-only soft support writes recover underediting without broad leakage',
    )
