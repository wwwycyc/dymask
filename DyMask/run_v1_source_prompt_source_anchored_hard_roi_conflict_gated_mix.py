...
from DyMask.conflict_gated_feature_mix import ConflictGatedFeatureMixConfig
...
from DyMask.v1_source_prompt_source_anchored_hard_roi_conflict_gated_mix import (
    V1SourcePromptSourceAnchoredHardRoiConflictGatedMixEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--mix-inside-rewrite-gain-strength", type=float, default=0.05)
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_conflict_gated_mix_runs/sp_anchor_conflict_gated"
    if "--piebench-path" not in argv_list:
        args.piebench_path = "assets/PIE-Bench"
    if args.phase == "custom" and "--methods" not in argv_list:
        args.methods = ["full_dynamic_mask"]

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_sp_anchor_conflict_gated"
        if config.phase != "custom"
        else "sp_anchor_conflict_gated"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi_conflict_gated_mix.md"))
    ...
    if not 0.0 <= float(args.mix_start_ratio) <= 1.0:
        raise ValueError("mix-start-ratio must be within [0, 1]")
    if not 0.0 <= float(args.mix_end_ratio) <= 1.0:
        raise ValueError("mix-end-ratio must be within [0, 1]")
    if float(args.mix_start_ratio) > float(args.mix_end_ratio):
        raise ValueError("mix-start-ratio must be <= mix-end-ratio")
    if not 0.0 <= float(args.mix_inside_rewrite_gain_strength) <= 1.0:
        raise ValueError("mix-inside-rewrite-gain-strength must be within [0, 1]")
    if not 0.0 <= float(args.mix_outside_source_strength) <= 1.0:
        raise ValueError("mix-outside-source-strength must be within [0, 1]")

    feature_mix_config = ConflictGatedFeatureMixConfig(
        start_ratio=float(args.mix_start_ratio),
        end_ratio=float(args.mix_end_ratio),
        inside_rewrite_gain_strength=float(args.mix_inside_rewrite_gain_strength),
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_hard_roi_conflict_gated_mix_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": (
                "Hard ROI editing with outside-roi source anchoring and conflict-gated decoder feature "
                "routing that applies source pull outside ROI and source-target conflict gated rewrite "
                "gain inside ROI"
            ),
            ...
            "conflict_gated_feature_mix": {
                ...
                "inside_rewrite_gain_strength": feature_mix_config.inside_rewrite_gain_strength,
                ...
                "inside_gain_formula": "lambda_in * roi * normalized_mean_abs(F_tar - F_src)",
                "outside_pull_formula": "lambda_out * (1-roi) * (F_src - F_tar)",
            },
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi_conflict_gated_mix",
        ...
        conclusion="samples and conflict-gated mix config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiConflictGatedMixEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored hard-roi conflict-gated mix run finished",
        next_step="compare against hard-roi baseline and proedit-like mix",
    )
    editor.close()
