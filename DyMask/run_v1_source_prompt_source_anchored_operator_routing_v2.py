...
from DyMask.v1_source_prompt_source_anchored_operator_routing_v2 import (
    V1SourcePromptSourceAnchoredOperatorRoutingV2Editor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--preserve-gate-power", type=float, default=1.0)
    parser.add_argument("--interiority-bias", type=float, default=1.0)
    parser.add_argument("--boundary-bias", type=float, default=1.0)
    parser.add_argument("--disable-stabilize-operator", action="store_true")
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_operator_routing_v2_runs/sp_operator_routing_v2"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_operator_routing_v2" if config.phase != "custom" else "sp_operator_routing_v2"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_operator_routing_v2.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_operator_routing_v2",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI support with outside-roi source anchoring and roi-internal epsilon-level operator routing",
            ...
            "preserve_gate_power": args.preserve_gate_power,
    ...
    logger.log(
        stage="source_prompt_source_anchored_operator_routing_v2",
        ...
        conclusion="samples and source-anchored operator-routing-v2 config saved",
    ...
    editor = V1SourcePromptSourceAnchoredOperatorRoutingV2Editor(
        pipe,
        config,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
        transform_attention_weight=args.transform_attention_weight,
    ...
    logger.log(
        ...
        conclusion="source-anchored operator-routing-v2 run finished",
