...
from DyMask.source_attention_injection import SourceSelfAttentionInjectionConfig
from DyMask.utils import make_timestamped_run_dir, save_csv_records, save_json
from DyMask.v1_source_prompt_source_anchored_hard_roi_self_attention_injection import (
    V1SourcePromptSourceAnchoredHardRoiSelfAttentionInjectionEditor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--inject-start-ratio", type=float, default=0.30)
    parser.add_argument("--inject-end-ratio", type=float, default=1.0)
    parser.add_argument("--inject-strength", type=float, default=1.0)
    parser.add_argument("--inject-locations", nargs="+", default=["up"])
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_self_inject_runs/sp_anchor_self_inject"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_anchor_self_inject" if config.phase != "custom" else "sp_anchor_self_inject"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_hard_roi_self_injection.md"))
    ...
    injection_config = SourceSelfAttentionInjectionConfig(
        ...
        locations=tuple(str(item) for item in args.inject_locations),
    )

    save_json(
        run_dir / "variant.json",
        {
            "variant_name": "source_prompt_source_anchored_hard_roi_self_attention_injection_v1",
            "ddim_inversion_prompt_mode": "source_prompt",
            "reference_branch_prompt_mode": "source_prompt",
            "attention_prompt_mode": "target_prompt",
            "mechanism": "Hard ROI editing with outside-roi source anchoring and source self-attention injection",
            ...
            "self_attention_injection": {
                ...
                "locations": list(injection_config.locations),
            },
    ...
    logger.log(
        stage="source_prompt_source_anchored_hard_roi_self_attention_injection",
        ...
        conclusion="samples and self-attention-injection config saved",
    ...
    editor = V1SourcePromptSourceAnchoredHardRoiSelfAttentionInjectionEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="source-anchored hard-roi self-attention-injection run finished",
        next_step="inspect summary tables and visual edit strength",
    )
