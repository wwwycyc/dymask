...
from DyMask.v1_source_prompt_source_anchored_support_c6c import V1SourcePromptSourceAnchoredSupportC6CEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c6c"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c6c"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c6c"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c6c.md"))
    ...
    save_json(
        run_dir / "variant.json",
        {
            ...
            "consistency_floor": float(args.consistency_floor),
            "roi_trust_max_strength": float(args.roi_trust_max_strength),
            "roi_trust_start_ratio": float(args.roi_trust_start_ratio),
            "roi_trust_full_ratio": float(args.roi_trust_full_ratio),
            "mechanism": (
                "C6c keeps the C6 consistency floor and late fallback schedule, but converts the late branch "
                "into a support-gap recovery path that activates only where support_state remains above the "
                "current mask. This is meant to refill under-edited holes without reopening the whole roi."
            ),
            ...
            "background_anchor": (
                "A_t = lerp(roi_soft, roi_hard, h_t); "
                "R_t = alpha_t * roi_soft * max(sqrt(discrepancy * dynamic_mask), beta_t * sqrt(relu(support_state - mask))) "
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c6c",
        ...
        conclusion="samples and C6c source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC6CEditor(
        pipe,
        config,
        ...
        consistency_floor=args.consistency_floor,
        roi_trust_max_strength=args.roi_trust_max_strength,
        roi_trust_start_ratio=args.roi_trust_start_ratio,
        roi_trust_full_ratio=args.roi_trust_full_ratio,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_source_anchored_support_c6c_confidence_roi_agreement_v1",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="C6c source-anchored temporal-support prompt run finished",
        next_step="inspect whether recovery-only roi support helps under-edited holes relative to C6/C6b",
    )
