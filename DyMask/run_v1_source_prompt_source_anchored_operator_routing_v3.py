...
from DyMask.v1_source_prompt_source_anchored_operator_routing_v3 import (
    V1SourcePromptSourceAnchoredOperatorRoutingV3Editor,
)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--boundary-gate-power", type=float, default=1.0)
    args = parser.parse_args(argv_list)
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_operator_routing_v3_runs/sp_operator_routing_v3"

    config = build_config(args)
    run_prefix = f"{config.phase}_sp_operator_routing_v3" if config.phase != "custom" else "sp_operator_routing_v3"
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_operator_routing_v3.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_operator_routing_v3",
            ...
            "boundary_gate_power": args.boundary_gate_power,
    ...
    logger.log(
        stage="source_prompt_source_anchored_operator_routing_v3",
        ...
        conclusion="samples and source-anchored operator-routing-v3 config saved",
    ...
    editor = V1SourcePromptSourceAnchoredOperatorRoutingV3Editor(
        pipe,
        config,
        transform_discrepancy_weight=args.transform_discrepancy_weight,
        transform_attention_weight=args.transform_attention_weight,
    ...
    logger.log(
        ...
        conclusion="source-anchored operator-routing-v3 run finished",
