...
class V1SourcePromptValidatedRevocableStateEditor(V1SourcePromptEditor):
    def __init__(
        self,
        pipe,
        config,
    ...
    def _support_consistent_map(
        ...
        if method_name in {"discrepancy_attention", "full_dynamic_mask"}:
            return torch.clamp(
                self.support_lambda * attention + (1.0 - self.support_lambda) * dynamic_mask,
                0.0,
                1.0,
            )
        ...
        try:
            ...
            if not self._uses_support_state(method_name):
                if method_name == "target_only":
                    ...
                    effective_mask, _aux_tensor = builder.build(
                        eps_src,
                        target_noise,
                        latents,
                        source_latents,
                ...
                _support_consistent, _support_inconsistent, _support_probability, support_evidence = self._support_evidence(
                    method_name,
                    dynamic_mask,
                    aux_tensor,
                )
        ...
        for step_idx, timestep in enumerate(self.pipe.scheduler.timesteps):
            ...
            if not self._uses_support_state(method_name):
                if method_name == "target_only":
                    ...
                    effective_mask, aux_tensor = builder.build(
                        eps_src,
                        target_noise,
                        latents,
                        source_latent,
                ...
                support_consistent, support_inconsistent, support_probability, support_evidence = self._support_evidence(
                    method_name,
                    dynamic_mask,
                    aux_tensor,
                )
