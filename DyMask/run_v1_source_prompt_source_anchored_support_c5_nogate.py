...
from DyMask.v1_source_prompt_source_anchored_support_c5_nogate import V1SourcePromptSourceAnchoredSupportC5NoGateEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c5_nogate"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c5_nogate"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c5_nogate"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c5_nogate.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c5_nogate_local_anchor_v1",
            ...
            "mechanism": (
                "Baseline adaptive temporal support plus local soft-roi anchor relaxation without confidence gating. "
                "Support memory stays baseline; source anchoring is relaxed uniformly inside soft roi."
            ),
            ...
            "background_anchor": (
                ...
                "R_t = alpha_t * roi_soft; "
                "A'_t = lerp(A_t, 1, R_t); "
                "z_{t-1} = A'_t * z_{t-1}^{edit} + (1-A'_t) * z_{t-1}^{src}"
            ),
            "ablation": "remove confidence gating while keeping local anchor relaxation",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c5_nogate",
        ...
        conclusion="samples and C5 no-gate source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC5NoGateEditor(
        pipe,
        config,
    ...
    logger.log(
        ...
        conclusion="C5 no-gate source-anchored temporal-support prompt run finished",
        next_step="compare C5 with and without confidence gating",
    )
