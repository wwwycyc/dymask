...
from DyMask.v1_source_prompt_source_anchored_support_c4 import V1SourcePromptSourceAnchoredSupportC4Editor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = 'scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c4'

    config = build_config(args)
    run_prefix = (
        f'{config.phase}_source_prompt_source_anchored_support_c4'
        if config.phase != 'custom'
        else 'v1_source_prompt_source_anchored_support_c4'
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path('log_source_anchored_support_c4.md'))
    ...
    save_json(
        run_dir / 'variant.json',
        {
            ...
            'support_soft_roi_blend': float(args.support_soft_roi_blend),
            'anchor_enable_start_ratio': float(args.anchor_enable_start_ratio),
            'anchor_enable_full_ratio': float(args.anchor_enable_full_ratio),
            'support_soft_enable_start_ratio': float(args.support_soft_enable_start_ratio),
            'support_soft_enable_full_ratio': float(args.support_soft_enable_full_ratio),
            'support_floor_start_ratio': float(args.support_floor_start_ratio),
            'support_floor_full_ratio': float(args.support_floor_full_ratio),
            'support_floor_max': float(args.support_floor_max),
            'mechanism': 'C3 plus a late roi floor that injects a small minimum edit drive inside the scheduled support region so weak dynamic evidence does not collapse into under-editing',
            'support_update': 'S_t = rho * S_{t-1} + (1-rho) * (dynamic_mask * lerp(roi_hard, roi_soft, beta * q_t)), S_0 = dynamic_mask * lerp(roi_hard, roi_soft, beta * q_t)',
            'support_soft_schedule': 'q_t = 0 before support-soft start, cosine-ramp to 1 by support-soft full',
            'adaptive_mask': 'M_t = max(lerp(S_t, roi_soft, w_t), F_t), where F_t is a late-stage roi floor',
            'support_floor': 'F_t = alpha_t * lerp(roi_hard, roi_soft, beta * q_t), alpha_t is 0 before support-floor start and cosine-ramps to support_floor_max',
            'background_anchor': "A_t = lerp(roi_soft, roi_hard, h_t); g_t delays anchoring; A'_t = lerp(1, A_t, g_t); z_{t-1} = A'_t * z_{t-1}^{edit} + (1-A'_t) * z_{t-1}^{src}",
    ...
    logger.log(
        stage='source_prompt_source_anchored_support_c4',
        ...
        conclusion='samples and C4 source-anchored support config saved',
    ...
    editor = V1SourcePromptSourceAnchoredSupportC4Editor(
        pipe,
        config,
        ...
        support_soft_roi_blend=args.support_soft_roi_blend,
        anchor_enable_start_ratio=args.anchor_enable_start_ratio,
        anchor_enable_full_ratio=args.anchor_enable_full_ratio,
        support_soft_enable_start_ratio=args.support_soft_enable_start_ratio,
        support_soft_enable_full_ratio=args.support_soft_enable_full_ratio,
        support_floor_start_ratio=args.support_floor_start_ratio,
        support_floor_full_ratio=args.support_floor_full_ratio,
        support_floor_max=args.support_floor_max,
    ...
    logger.log(
        ...
        conclusion='C4 source-anchored temporal-support prompt run finished',
        next_step='inspect whether the late roi floor reduces remaining under-editing without collapsing locality',
    )
