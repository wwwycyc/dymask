...
class V1SourcePromptValidatedSupportFactorizedEditor(V1SourcePromptHardRoiLockedEditor):
    def __init__(
        self,
        pipe,
        config,
    ...
    def _reference_prompt_metadata(self, sample: MaterializedSample) -> dict[str, object]:
        ...
        payload.update(
            {
                "variant": "source_prompt_validated_support_factorized_v1",
                "roi_mask_policy": "factorized validated discrepancy accumulation with roi-aware support probability and explicit credibility",
                ...
                "support_update": (
                    "S_t = rho * S_{t-1} + (1-rho) * (Norm(D_t) * Pi_t * C_t^cred), "
                    "M_t = sigmoid(alpha * (S_t - delta))"
                ),
                "support_probability": "Pi_t = sigmoid(kappa * (g_t - h_t))",
                "credibility_map": "C_t^cred = 1 - latent_drift",
            }
        )
        return payload
    ...
    def _credibility_map(self, aux_tensor: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    def _validated_components(
        ...
        credibility = self._credibility_map(aux_tensor)
        validation_map = support_probability * credibility
        support_evidence = discrepancy * validation_map
        return support_consistent, support_inconsistent, support_probability, credibility, validation_map, support_evidence
    ...
    def _probe_method_batch_memory(
        ...
        try:
            ...
            if method_name == "target_only":
                ...
                (
                    _support_consistent,
                    _support_inconsistent,
                    _support_probability,
                    _credibility,
                    _validation_map,
                    support_evidence,
    ...
    def _run_method_batch(
        ...
        for step_idx, timestep in enumerate(self.pipe.scheduler.timesteps):
            ...
            if method_name == "target_only":
                ...
                credibility = torch.ones_like(eps_src[:, :1])
                validation_map = torch.ones_like(eps_src[:, :1])
                ...
                aux_tensor["credibility"] = credibility
                aux_tensor["validation_map"] = validation_map
                ...
                trace_histories[sample_idx].append(
                    {
                        ...
                        "credibility_mean": float(credibility[sample_idx].mean().item()),
                        "validation_map_mean": float(validation_map[sample_idx].mean().item()),
