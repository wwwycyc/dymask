...
from DyMask.v1_source_prompt_source_anchored_support_c2_only import V1SourcePromptSourceAnchoredSupportC2OnlyEditor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not _has_output_root_arg(argv_list):
        args.output_root = "scratch_source_anchor_runs/dymask_v1_source_prompt_source_anchored_support_c2_only"

    config = build_config(args)
    run_prefix = (
        f"{config.phase}_source_prompt_source_anchored_support_c2_only"
        if config.phase != "custom"
        else "v1_source_prompt_source_anchored_support_c2_only"
    )
    run_dir = make_timestamped_run_dir(config.sampling.output_root, prefix=run_prefix)
    logger = MarkdownExperimentLogger(Path("log_source_anchored_support_c2_only.md"))
    ...
    save_json(
        ...
        {
            "variant_name": "source_prompt_source_anchored_support_c2_only_delayed_anchor_v1",
            ...
            "mechanism": "Baseline adaptive temporal support plus delayed source anchoring only",
    ...
    logger.log(
        stage="source_prompt_source_anchored_support_c2_only",
        ...
        conclusion="samples and C2-only source-anchored support config saved",
    ...
    editor = V1SourcePromptSourceAnchoredSupportC2OnlyEditor(
        pipe,
        config,
    ...
    for sample in run_samples:
        ...
        for result in method_results:
            row = {
                ...
                "variant": "source_prompt_source_anchored_support_c2_only_delayed_anchor_v1",
            }
            if metric_runner is not None:
    ...
    logger.log(
        ...
        conclusion="C2-only source-anchored temporal-support prompt run finished",
        next_step="inspect whether delayed anchoring alone reduces under-editing",
    )
